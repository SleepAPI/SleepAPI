...
// import { SetCover, SetCoverPokemonSetup } from '../../set-cover/set-cover';
// import {
...
//   it('shall calculate optimal produce for all optimal pokemon', async () => {
//     const data = await calculateOptimalProductionForSetCover(
//       {
...
//       1
//     );
//     const pinsirData = data.filter((entry) => entry.pokemonSet.pokemon === pokemon.PINSIR.name);
//     expect(pinsirData).toHaveLength(6);
//   });
// });

// describe('calculateMealContributionFor', () => {
//   it('shall calculate Gengars contribution and divide by 3 for slowpoke tail salad with size 3 team size', async () => {
//     const meal = salad.SLOWPOKE_TAIL_PEPPER_SALAD;

//     const allPokemonWithProduce = await calculateOptimalProductionForSetCover(
//       {
//         ...MOCKED_OPTIMAL_PRODUCTION_STATS,
//         islandBerries: berry.BERRIES,
//       },
...
//       .filter(
//         (pk) =>
//           hashPokemonCombination(pk.pokemonSet) ===
//           `${pokemon.GENGAR.name}:${[
...
//   it('shall calculate Gengars level 50 contribution and divide by 2 for spore mushroom curry with size 2 team size', async () => {
...
//       .filter((pk) => pk.pokemonSet.pokemon === pokemon.GENGAR.name)[0];
...
//     const producedTomato: SimplifiedIngredientSet = { amount: 8, ingredient: ingredient.SNOOZY_TOMATO.name };
...
//     const producedTomato = { amount: 4, ingredient: ingredient.SNOOZY_TOMATO.name };
...
//     const producedTomato: SimplifiedIngredientSet = { amount: 4, ingredient: ingredient.SNOOZY_TOMATO.name };
//     const producedFiller: SimplifiedIngredientSet = { amount: 4, ingredient: ingredient.FANCY_APPLE.name };
//     const expectedPunishment = 0.6;
...
//     const support: SimplifiedIngredientSet = { amount: 2, ingredient: ingredient.FANCY_APPLE.name };
...
//     const defaultProduce: SetCoverPokemonSetup[] = [raikou, dragonite, tyranitar];
...
//     const defaultProduce: SetCoverPokemonSetup[] = [raikou, dragonite, charizard, houndoom];
...
//     const defaultProduce: SetCoverPokemonSetup[] = [defaultJolteon];
...
//     const teams = memoizedSetCover.findOptimalCombinationFor(
//       simplifyIngredientSet(recipe.ingredients),
//       [currentPokemon],
//       5,
//       1000
//     );
...
//     },
//     averageIngredients: simplifyIngredientSet(average),
//     totalIngredients: simplifyIngredientSet(produced),
//     skillProcs,
//     berry: pokemon.berry.name,
//     skill: pokemon.skill.name,
//   };
// }
