...
import {
  ...
  emptyIngredientInventoryFlat,
  flatToBerrySet,
  flatToIngredientSet,
  getEmptyInventoryFlat,
  ingredient,
  ingredientSetToFlat,
  mainskill,
  multiplyProduce,
...
export class MemberState {
  ...
  private averageProduce: ProduceFlat = getEmptyInventoryFlat();
  ...
  constructor(params: {
    ...
    const pokemonIngredientListFlat: Float32Array = ingredientSetToFlat(member.pokemonWithIngredients.ingredientList);
  ...
  private averageCritMultiplier(iterations: number): number {
    if (!this.cookingState) {
      return AVERAGE_WEEKLY_CRIT_MULTIPLIER;
    } else {
      return this.cookingState.results(iterations).critInfo.averageCritMultiplierPerCook;
    }
  }
  ...
  #activateIngredientMagnet(): SkillActivation {
    ...
    const magnetIngredients: Float32Array = emptyIngredientInventoryFlat().fill(
    ...
    this.totalProduce.ingredients = flatToIngredientSet(
      ingredientSetToFlat(this.totalProduce.ingredients)._mutateCombine(magnetIngredients, (a, b) => a + b)
    );
