...
<script lang="ts">
import { maxCarrySize, MOCK_POKEMON, type Pokemon, type PokemonInstanceExt } from 'sleepapi-common'
  ...
  watch: {
    pokemon: {
      handler(newPokemon: Pokemon, oldPokemon: Pokemon) {
        ...
        const loadFromExisting = this.pokemonInstance.carrySize > 0 && oldPokemon.name === MOCK_POKEMON.name
